...
// it loads the ggxrd_freecam_dll.dll on startup automatically
#include <iostream>
#include <string>
#ifndef FOR_LINUX
#include <Windows.h>
...
void meatOfTheProgram() {
    ...
    std::string stringToWrite = "ggxrd_freecam_dll.dll";
...
{
    CrossPlatformCout <<
        "This program patches GuiltyGearXrd.exe executable to permanently launch this freecam mod when it starts.\n"
        ...
        "(!) The ggxrd_freecam_dll.dll must be placed in the same folder as the game executable in order to get loaded by the game. (!)\n"
