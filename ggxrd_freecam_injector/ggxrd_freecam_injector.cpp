#include "pch.h"
#include "ggxrd_freecam_injector.h"
#include "ConsoleEmulator.h"
#include "ggxrd_freecam_injector_common.h"
...
UINT windowClassNameResourceId = IDC_GGXRDFREECAMINJECTOR;
LPCWSTR windowIconId = MAKEINTRESOURCEW(IDI_GGXRDFREECAMINJECTOR);
LPCWSTR windowMenuName = MAKEINTRESOURCEW(IDC_GGXRDFREECAMINJECTOR);
LPCWSTR windowAcceleratorId = MAKEINTRESOURCEW(IDC_GGXRDFREECAMINJECTOR);
...
bool parseArgs(int argc, LPWSTR* argv, int* exitCode) {
	for (int i = 0; i < argc; ++i) {
		if (_wcsicmp(argv[i], L"/?") == 0
				...
				|| _wcsicmp(argv[i], L"-help") == 0) {
			MessageBoxA(NULL,
				"Injector for ggxrd_freecam_dll for Guilty Gear Xrd Rev2 version 2211."
				" Arguments:\n"
